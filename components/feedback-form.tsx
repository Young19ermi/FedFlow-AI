...
// import { Paperclip, Mic, Send, Phone, User } from "lucide-react";
...
//     { text: string; isUser: boolean; fileName?: string }[]
//   >([]);
...
//         "Would you like to call HCX Resort support now?"
//       );
//       if (confirmCall) {
...
//       <div className="rounded-md backdrop-blur-sm bg-white/30 border border-gray-100 shadow-sm p-4 md:p-6">
...
//             Hello! How can we make your stay at HCX Resort more enjoyable? Feel
//             free to share any feedback or requests.
//           </p>
//         </div>
...
export default function FeedbackForm() {
  ...
  const webhookUrl =
    "https://hcx-resort.app.n8n.cloud/webhook-test/3c2b4e29-6a6d-4431-8854-f718a99cfca5"; // Replace with actual webhook URL
  ...
  // Function to send feedback and fetch response
  const handleSendFeedback = useCallback(async () => {
    ...
    try {
      const response = await axios.post(webhookUrl, {
        message: feedback, // Send the user feedback to the server
      });
      ...
      setMessages((prev) => [
        ...prev,
        { text: "Oops, I’m having trouble!", isUser: false },
      ]);
    } finally {
      ...
      <div className="rounded-md backdrop-blur-sm bg-white/30 border border-gray-100 shadow-sm p-4 md:p-6">
        <div className="flex justify-between gap-12 mb-6">
          ...
          <Button
            ...
            className="rounded-full border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700"
            onClick={() => {
              window.location.href = "tel:+18004429876";
            }}
          >
            <Phone className="h-4 w-4 mr-2" />
            Emergency Call
        ...
        <div className="mb-4">
          <p className="text-gray-700">
            Hello! How can we make your stay at Resort more enjoyable? Feel free
            to share any feedback or requests.
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-3 right-3 flex items-center gap-2">
            ...
            <Button
              ...
              className="h-8 w-8 rounded-full bg-black text-white hover:bg-gray-800"
              onClick={handleSendFeedback}
              disabled={(feedback.trim() === "" && !attachedFile) || isLoading} // Disable while loading
            >
              <Send className="h-4 w-4" />
            </Button>
          </div>
